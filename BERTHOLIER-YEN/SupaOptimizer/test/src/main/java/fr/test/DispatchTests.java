...
import static fr.umlv.SupaOptimizer.dispatch;
...
public class DispatchTests {
    ...
    sealed interface Itf {
        int m(int i) ;
        record A() implements Itf { public int m(int i) { return i + 1; } }
        record B() implements Itf { public int m(int i) { return i + 2; } }
    }
    sealed interface Sole {
        double m(double d);
        record Impl() implements Sole { public double m(double d) { return d + 2.0; } }
    }
